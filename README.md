...
- **[dsh-fast](https://github.com/PerryLink/dsh-fast)**（[@PerryLink](https://github.com/PerryLink) · 2026-08-17）— 只读零侵入性能诊断：会话加载耗时/spill 命中/压缩统计/上下文注入体量/缓存命中率；/fast 命令 + fast_report 工具，异步采样不占模型路径。适合想看清 DSH 运行性能瓶颈、又不想改动行为的用户。

...
- **[dsh-skill-pack-security](https://github.com/PerryLink/dsh-skill-pack-security)**（[@PerryLink](https://github.com/PerryLink) · 2026-08-16）— 安全审计方法论技能包 + plugin_vet 供应链门禁：八个 agent 技能（密钥扫描、依赖审计、供应链评审、提示注入审查、审计总编排、威胁建模、漏洞情报、事件响应），中英双版本；经 npm provider 包（@perrylink/dsh-skill-pack-security-provider 2.0.0）一键挂载，并注册自动化 plugin_vet 安装前扫描（license/SBOM/commit 锁定/恶意模式/五维风险卡），默认 warn 可切 deny。

...
- **[dsh-test-drive](https://github.com/PerryLink/dsh-test-drive)**（[@PerryLink](https://github.com/PerryLink) · 2026-08-17）— 自动装机实测矩阵：在隔离的临时 DSH_HOME 里对插件执行安装→patch 校验→启动冒烟→卸载→清理，输出结构化 dsh-test-drive/v1 结果供评分管线消费；适合需要可复现安装实测的插件作者与维护者。

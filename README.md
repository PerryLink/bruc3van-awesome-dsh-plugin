...
- **[dsh-defend](https://github.com/PerryLink/dsh-defend)**（[@PerryLink](https://github.com/PerryLink) · 2026-08-16）— 注入/越狱/密钥泄露检测 + 危险删除门禁：Aho-Corasick 引擎在用户消息、工具参数、工具结果三处按 allow/ask/block 拦截，附脱敏 defend/detection 审计事件、defend_report 工具与 /defend 命令，并拒绝工作区外的递归删除命令；无网络依赖。

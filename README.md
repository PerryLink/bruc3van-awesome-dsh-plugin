...
- **[dsh-local-ai](https://github.com/PerryLink/dsh-local-ai)**（[@PerryLink](https://github.com/PerryLink) · 2026-08-17）— Ollama 本地模型接入：ollama_list/pull/remove/show 与健康检查，以官方 LlmAdapter 注册 Ollama 路由与 model_route 分流（离线优先/长文本/隐私），失败回退云端；/ollama 命令一键总览。适合想在本地跑模型、看重离线与隐私的 DSH 用户。

...
- **[dsh-defend](https://github.com/PerryLink/dsh-defend)** ([@PerryLink](https://github.com/PerryLink) · 2026-08-16) — Prompt-injection, jailbreak, and secret-leak detection plus a destructive-delete guard: an Aho-Corasick engine over ported rule sets gates user messages, tool arguments, and tool results with allow/ask/block tiers, with sanitized audit events, a defend_report tool, a /defend command, and a guard against recursive deletes outside the workspace; no network dependency.

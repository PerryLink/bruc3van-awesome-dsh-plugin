...
- **[dsh-test-drive](https://github.com/PerryLink/dsh-test-drive)** ([@PerryLink](https://github.com/PerryLink) · 2026-08-17) — Isolated install-smoke-uninstall test drives for DSH plugins: runs install → patch check → launch smoke → uninstall → cleanup inside a throwaway DSH_HOME and emits structured dsh-test-drive/v1 results for scoring pipelines — for plugin authors and maintainers who need reproducible install verification.

...
- **[dsh-session-pin](https://github.com/PerryLink/dsh-session-pin)** ([@PerryLink](https://github.com/PerryLink) · 2026-08-16) — Pins sessions and workspaces to the sidebar top (row badges, per-pin colors, header toggle, pinned panel, durable settings namespace); 0.4.0 adds the session navigation organizer — pin groups (boards), tags and saved filter views, read-only sanitized health summaries, and /goto fuzzy jump, all browser-local.

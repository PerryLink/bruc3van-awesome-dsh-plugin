...
- **[dsh-local-ai](https://github.com/PerryLink/dsh-local-ai)** ([@PerryLink](https://github.com/PerryLink) · 2026-08-17) — Ollama local-model adapter: ollama_list/pull/remove/show plus health checks, registers an Ollama route via the official LlmAdapter with model_route rules (offline-first / long-text / privacy) and cloud fallback; /ollama command for a one-shot overview. For DSH users who want local models with offline and privacy priority.

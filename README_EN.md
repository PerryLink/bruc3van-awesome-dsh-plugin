...
- **[dsh-session-sync](https://github.com/PerryLink/dsh-session-sync)** ([@PerryLink](https://github.com/PerryLink) · 2026-08-16) — Cross-device session sync: a dedicated git mirror with append-only keep-both conflict resolution (local version kept, remote preserved as fork files — never silently overwritten), with a /sync command and sync_status/sync_pull/sync_push tools.

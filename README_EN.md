...
- **[dsh-fast](https://github.com/PerryLink/dsh-fast)** ([@PerryLink](https://github.com/PerryLink) · 2026-08-17) — Read-only performance diagnostics: session load timing, spill hits, compaction stats, context-injection volume, and cache-hit rate via the /fast command and fast_report tool, sampled asynchronously off the model path. For DSH users who want to see performance bottlenecks without changing behavior.
